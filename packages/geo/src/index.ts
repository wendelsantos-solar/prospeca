// @leads/geo — pure geographic helpers. No runtime deps, no React/Supabase/fetch.
// Shared by domain (dedup), providers (Overpass bbox), edge functions.
...
/** Distance in kilometers, rounded to one decimal. */
export function haversineKm(a: LatLng, b: LatLng): number {
  return Number((haversineMeters(a, b) / 1000).toFixed(1));
}

/** True latitude/longitude range. Rejects NaN and out-of-range values. */
...
/**
 * Bounding box around a center point for a given radius in meters.
 * Longitude delta is corrected by latitude (converges toward the poles).
 */
...
/**
 * Decode a PostGIS geography/geometry point.
 * PostgREST serializes as hex EWKB ("0101000020E6100000...") — NOT GeoJSON.
 * Accepts EWKB hex or a GeoJSON Point; returns [longitude, latitude].
 * (Ported from supabase/functions/_shared/geo.ts to consolidate the logic.)
 */
...
/** Build a "POINT(lng lat)" WKT string, or null when coords are invalid. */
