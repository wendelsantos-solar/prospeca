// Deduplication — multi-signal, deterministic, idempotent.
...
/** Confidence that two records are the same business (0..1). */
...
export interface DedupCluster {
  canonicalId: string;
  memberIds: string[];
  reviewIds: string[]; // candidates that matched only at review confidence
}

/**
 * Cluster records into deduplicated groups. Deterministic (input order stable)
 * and idempotent: running it on already-clustered canonical records yields
 * one-member clusters. High-confidence pairs merge; review-confidence pairs are
 * attached as reviewIds but NOT merged.
 */
